import pytest
from authentication.services.auth import register, NAME_MAX_LENGTH, NAME_MIN_LENGTH, EMAIL_MAX_LENGTH
...
class TestUserServiceRegister:
    ...
    def invalid_data(self):
        return {"name": "sd", "email": "ldkasdkadokasdkasdopakdopaskdpoasdpoaskdpokasdpoksadpo"}

    def test_invalid_data(self, invalid_data):
        response, status_code = register(
            **invalid_data, Model=UserModel)
    ...
    def test_max_name_length(self, valid_data):
        response, status_code = register(
            name="ldkasdkadokasdkasdopakdopaskdpoasdpoaskdpokasdpoksadpo", email=valid_data.get("email"), Model=UserModel)
